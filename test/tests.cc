...
TEST(Bignum, InitStartsEmptyNumber)
{
  ...
  EXPECT_EQ(number.number, std::vector<uint32_t>());
}
...
TEST(Bignum, InitStartsNotEmptyNumberAsInt)
{
  ...
  EXPECT_EQ(number_positive.number, std::vector<uint32_t>({
    92831231, 391283
  }));
  ...
  EXPECT_EQ(number_positive_zero.number, std::vector<uint32_t>({
    0
  }));
  ...
  EXPECT_EQ(number_negative.number, std::vector<uint32_t>({
    9847129, 28192891, 1
  }));
  ...
  EXPECT_EQ(number_negative_one.number, std::vector<uint32_t>({
    1
  }));
  ...
  EXPECT_EQ(number_positive_one.number, std::vector<uint32_t>({
    1
  }));
}
...
TEST(Bignum, InitStartsNotEmptyNumberAsCString)
{
  ...
  EXPECT_EQ(number_positive_big.number, std::vector<uint32_t>({
  ...
  EXPECT_EQ(number_negative_big.number, std::vector<uint32_t>({
            0,         0,         0,         0,         0,
